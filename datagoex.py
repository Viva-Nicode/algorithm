...
ServiceKey = "nrridzQSepOgdGwwhEwUav3VPi2uPDwFwMaXtOpdbqI3XptXcocTnOCFuLoJ2K9tDTO2B4k7IK9b6DQyB9zwLA%3D%3D"
        ...
        for month in range(1, 13):
            ...
            if jsonData["response"]["header"]["resultMsg"] == "OK":
                if jsonData["response"]["body"]["items"] == "":
                    ...
                    print(
                        "데이터 없음.... ₩n 제공되는 통계 데이터는 85년 65월까지입니다. "
...
def main():
    ...
    with open(
        "./%s_%s_%d_%s.json" % (natName, ed, nStartYear, dataEND), "W", encoding="utf8"
    ) as outfile:
    ...
    result_df.to_csv(
        "./%s_%s_%d_%s.csv" % (natName, ed, nStartYear, dataEND),
        index=False,
        encoding="cp949",
    )
